...
export const People = () => {
    ...
    const handlePeople = (url) => {
        actions.settingPeopleUrl(url);
    };
    return (
        <>
            {!store.people ? <h1>no hay info</h1> :
                    ...
                    {store.people.map((item, id) =>
                        <div key={id} className="card d-flex justify-content-center m-2 bg-dark" style={{ width: '18rem' }}>
                            <div className="card-body">
                                <img src={`https://starwars-visualguide.com/assets/img/characters/${item.uid}.jpg`} onError={imgError} className="card-img-top"></img>
